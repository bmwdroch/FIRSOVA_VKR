...
flowchart TD
    A[Обучающие данные]
    B1[Бутстрэп выборка 1]
    B2[Бутстрэп выборка 2]
    BN[Бутстрэп выборка N]
    T1[Дерево решений 1]
    T2[Дерево решений 2]
    TN[Дерево решений N]
    P1[Прогноз 1]
...
```mermaid
flowchart TD
    A["Начальное предсказание F₀(x) = const"]
    A --> B["Итерация 1"]
    B --> C["Вычисление остатков: r₁ = y - F₀(x)"]
    C --> D["Обучение дерева h₁(x) на остатках r₁"]
    D --> E["Обновление модели: F₁(x) = F₀(x) + α₁·h₁(x)"]
    E --> F["Итерация 2"]
    F --> G["Вычисление остатков: r₂ = y - F₁(x)"]
    G --> H["Обучение дерева h₂(x) на остатках r₂"]
    H --> I["Обновление модели: F₂(x) = F₁(x) + α₂·h₂(x)"]
    I --> J["..."]
    J --> K["Итерация T"]
    K --> L["Вычисление остатков: rₜ = y - Fₜ₋₁(x)"]
    L --> M["Обучение дерева hₜ(x) на остатках rₜ"]
    M --> N["Обновление модели: Fₜ(x) = Fₜ₋₁(x) + αₜ·hₜ(x)"]
    N --> O["Финальная модель: F(x) = F₀(x) + α₁·h₁(x) + α₂·h₂(x) + ... + αₜ·hₜ(x)"]
```
